...
export function AdminComplaintsPage() {
  ...
  const priorityLabel = (p: number) => {
    if (p >= 4) return { label: 'بحرانی', color: 'text-red-600 bg-red-50' };
    if (p >= 3) return { label: 'بالا', color: 'text-orange-600 bg-orange-50' };
    if (p >= 2) return { label: 'متوسط', color: 'text-amber-600 bg-amber-50' };
    return { label: 'پایین', color: 'text-green-600 bg-green-50' };
  };
                  ...
                  <div className="px-4 pb-4 border-t border-gray-100 pt-3 space-y-3 animate-fade-in">
                    ...
                    {c.ai_analysis && (
                      <div className="p-3 bg-purple-50 rounded-lg border border-purple-100">
                        <p className="flex items-center gap-1.5 text-xs font-semibold text-purple-700 mb-1">
                          <Sparkles size={13} />
                          تحلیل هوش مصنوعی:
                        </p>
                        <p className="text-xs text-purple-700">{c.ai_analysis}</p>
                      </div>
                    )}

                    {c.admin_response && (
                      <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                        <p className="text-xs font-semibold text-blue-700 mb-1">پاسخ مدیر:</p>
