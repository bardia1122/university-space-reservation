...
export function AdminComplaintsPage() {
  ...
  const priorityLabel = (p: number) => {
    if (p >= 4) return { label: 'بحرانی', color: 'text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10' };
    if (p >= 3) return { label: 'بالا', color: 'text-orange-600 bg-orange-50' };
    if (p >= 2) return { label: 'متوسط', color: 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10' };
    return { label: 'پایین', color: 'text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-500/10' };
  };
              ...
              <div key={c.id} className="card overflow-hidden">
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="font-semibold text-gray-900 dark:text-slate-100 truncate">{c.title}</h3>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {isExpanded ? <ChevronUp size={16} className="text-gray-400 dark:text-slate-500" /> : <ChevronDown size={16} className="text-gray-400 dark:text-slate-500" />}
                    </div>
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 border-t border-gray-100 dark:border-slate-800 pt-3 space-y-3 animate-fade-in">
                    <p className="text-sm text-gray-700 dark:text-slate-200 leading-relaxed">{c.description}</p>
                    ...
                    {c.ai_analysis && (
                      <div className="p-3 bg-purple-50 dark:bg-purple-500/10 rounded-lg border border-purple-100">
                        ...
                        <p className="text-xs text-purple-700 dark:text-purple-300">{c.ai_analysis}</p>
                      </div>
                    )}

                    {c.admin_response && (
                      <div className="p-3 bg-blue-50 dark:bg-blue-500/10 rounded-lg border border-blue-100 dark:border-blue-500/30">
                        <p className="text-xs font-semibold text-blue-700 dark:text-blue-300 mb-1">پاسخ مدیر:</p>
                        <p className="text-sm text-blue-800 dark:text-blue-300">{c.admin_response}</p>
                        {c.responded_at && (
                          <p className="text-xs text-blue-400 mt-1">{c.responded_at.slice(0, 10)}</p>
                        )}
                      </div>
                    )}
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <Button variant="secondary" onClick={() => setRespondId(null)}>انصراف</Button>
          </div>
        </div>
      </Modal>
    </div>
